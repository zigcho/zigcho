...
- baseline: 1.25x pp for every supported source, ruleset and namespace
...
the extra modifiers combine, then clamp between 0.9x and 1.1x before the baseline. the final multiplier stays between 1.125x and 1.375x. DT and NC at the same rate get the same adjustment. RX and AP keep their upstream formula differences; they don't get another bonus just for being RX or AP. existing miss, accuracy and mod penalties stay in the underlying calculation.
...
existing scores need one recalculation from their saved maps, hit results and mods. the changed engine marker makes release activation request that rebuild, with the normal backup and rollback checks. until that release is approved and activated, live scores stay unchanged.
